    ...
    <div class="biz-main">
        ...
        <header class="biz-topbar">
            <div class="d-flex align-items-center gap-3">
                ...
                <h1 class="biz-page-title">Create New Listing</h1>
            </div>
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h2 class="biz-banner-title" style="color: var(--biz-dark);">List Your Business</h2>
                <p class="text-muted" style="font-size: 0.9rem;">Fill in the details below to create a new business listing. Fields marked with <span class="text-danger">*</span> are required.</p>
            </div>
        ...
        <!-- Create Listing Form -->
        <form method="POST" action="{{ route('business.storee') }}" enctype="multipart/form-data" class="needs-validation" novalidate>
            @csrf
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select id="year_established"
                                        ...
                                        @for ($year = date('Y'); $year >= 1800; $year--)
                                            <option value="{{ $year }}" {{ old('year_established') == $year ? 'selected' : '' }}>
                                                {{ $year }}
                                            </option>
                                        @endfor
                                    </select>
                                                ...
                                                <td class="text-center">
                                                    <div class="form-check form-switch d-flex justify-content-center mb-0">
                                                        <input class="form-check-input day-closed-checkbox"
                                                               ...
                                                               value="1"
                                                               {{ old('opening_hours.' . $loop->index . '.is_closed') ? 'checked' : '' }}>
                                                    </div>
                                                </td>
                                                <td>
                                                    <input type="time"
                                                           ...
                                                           value="{{ old('opening_hours.' . $loop->index . '.opens_at') }}"
                                                           {{ old('opening_hours.' . $loop->index . '.is_closed') ? 'disabled' : '' }}>
                                                </td>
                                                <td>
                                                    <input type="time"
                                                           ...
                                                           value="{{ old('opening_hours.' . $loop->index . '.closes_at') }}"
                                                           {{ old('opening_hours.' . $loop->index . '.is_closed') ? 'disabled' : '' }}>
                                                </td>
                                            </tr>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (stateSelect && lgaSelect) {
                ...
                if (stateSelect.value) {
                    ...
                    // Restore previously selected LGA after loading
                    const oldLga = '{{ old('lga_id') }}';
                    if (oldLga) {
                        const checkLgaLoaded = setInterval(() => {
                            if (lgaSelect.options.length > 1) {
                                lgaSelect.value = oldLga;
                                clearInterval(checkLgaLoaded);
                            }
                        }, 100);
                    }
                }
            ...
            function setupImagePreview(inputId, previewId, placeholderIcon) {
                ...
                input.addEventListener('change', function (event) {
                    ...
                    if (!file) {
                        preview.innerHTML = placeholderIcon;
                        return;
                    }
